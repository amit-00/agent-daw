  ...
  return (
    <div role="group" aria-label={`${track.name} track`} className="sticky left-0 z-[3] col-start-1 flex min-w-0 flex-col border-r border-b border-white/10 bg-black" style={{ gridRow: row }}>
      ...
      <div className="mt-auto flex items-center gap-1 px-[9px] pb-2.5">
        <button disabled type="button" aria-label={`${track.muted ? "Unmute" : "Mute"} ${track.name}`} aria-pressed={track.muted} className={`h-[18px] w-[21px] rounded border text-[9px] ${track.muted ? "border-rose-400/45 bg-rose-400/20 text-rose-100" : "border-white/10 bg-zinc-900 text-zinc-500"}`}>M</button>
        <button disabled type="button" aria-label={`${track.soloed ? "Unsolo" : "Solo"} ${track.name}`} aria-pressed={track.soloed} className={`h-[18px] w-[21px] rounded border text-[9px] ${track.soloed ? "border-amber-300/45 bg-amber-300/15 text-amber-100" : "border-white/10 bg-zinc-900 text-zinc-500"}`}>S</button>
